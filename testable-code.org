...
to either mock or construct =Account= in addition to =Session= in all places =Session=
is a dependency. It will also be less than optimal when =Session= interface changes
by renaming =getAccount= to =getProfile= and it requires a change in many
places. While most IDEs will not have problem with it, but your commit history
will get noisy.
